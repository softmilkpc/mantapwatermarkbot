...
async def HelpWatermark(bot, cmd):
	...
	await cmd.reply_text(
		...
		parse_mode="Markdown",
		reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Channel", url="https://t.me/mantapvids"), InlineKeyboardButton("Group Video", url="https://ouo.io/qjnTY6")], [InlineKeyboardButton("Anime Channel", url="https://t.me/mantaphentai")], [InlineKeyboardButton("Live Streaming", url="https://t.me/mantapjozzzz")]]),
...
async def VidWatermarkAdder(bot, cmd):
	...
	if cmd.photo or (cmd.document and cmd.document.mime_type.startswith("image/")):
		...
		the_media = await bot.download_media(
			...
			progress_args=(
				"Sabar!! lagi Download kang ...",
				editable,
				c_time
			)
		)
		await editable.delete()
		await cmd.reply_text("Okeh berhasil ke save untuk Watermark divideo!\n\nSekarang kirimkan Video yang mau elu kasihkan watermarknya ke gw!")
		return
	else:
		pass
	...
	if not os.path.exists(watermark_path):
		await cmd.reply_text("Ga nge set watermark?\n\nKirim dulu Foto bentuk JPG atau PNG...")
		return
	file_type = cmd.video or cmd.document
	if not file_type.mime_type.startswith("video/"):
		await cmd.reply_text("Ini bukan Video GBLK!")
		return
	...
	if (int(file_size) > 2097152000) and (Config.ALLOW_UPLOAD_TO_STREAMTAPE is True) and (Config.STREAMTAPE_API_USERNAME != "NoNeed") and (Config.STREAMTAPE_API_PASS != "NoNeed"):
		...
		try:
			...
			print(f"Error: {e}")
			await editable.edit("Sorry, Something went wrong!\n\nCan't Upload to Streamtape. You can report at [Support Group](https://t.me/mantapvids).")
	...
	try:
		...
		print(f"Anjiirr kena spam..Tunggu dulu {e.x}s ...")
...
async def button(bot, cmd: CallbackQuery):
	...
	if "refreshmeh" in cb_data:
			...
			try:
				...
				if user.status == "kicked":
					await cmd.message.edit(
						text="Sorry Sir, You are Banned to use me. Contact my [Channel Support](https://t.me/mantapvids).",
						parse_mode="markdown",
						disable_web_page_preview=True
					)
					return
				...
				await cmd.message.edit(
					text="**Belum join ke channel gw kan ? Join dulu ke channel gw baru bisa pakai ini bot!**\n\nBiar ga Overload, Jadinya cuma Subscribers Channel yang bisa gunain gw!",
					reply_markup=InlineKeyboardMarkup(
						[
							[
		...
		await cmd.message.edit(
			text=Config.USAGE_WATERMARK_ADDER,
			parse_mode="Markdown",
			reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Channel", url="https://t.me/mantapvids"), InlineKeyboardButton("Group Video", url="https://ouo.io/qjnTY6")], [InlineKeyboardButton("Group Gratis", url="https://t.me/groupgratis")]]),
			disable_web_page_preview=True
		)
